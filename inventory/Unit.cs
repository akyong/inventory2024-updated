    ...
    public partial class Unit : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (button1.Text == "SAVE")
            {
                ...
                else
                {
                    ...
                    string queryCheck = "SELECT * FROM UNIT WHERE code=@codec and delete_flag='N'";
                    ...
                    if (reader.Read())
                    {
                        ...
                        DialogResult avaiable = MessageBox.Show("Kode Satuan " + codeTxt.Text + " sudah ada", "Duplicate", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    ...
                    else
                    {
                        try
                        {
                            string query = "INSERT INTO UNIT( code,name,description,delete_flag ) VALUES( @code,@names,@description,'N' )";
                        ...
                        string query2 = "select top 1 * from UNIT where delete_flag='N' ORDER BY ID DESC ";
            ...
            else if (button2.Text == "UPDATE")
            {
                ...
                else
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        if (id == idgudang.Text)
                        {
                            string query = "UPDATE UNIT SET code=@code, name=@name,description=@description WHERE id=@id";
                            Connection.command = new OleDbCommand(query, Connection.conn);
                            Connection.command.CommandType = CommandType.Text;
                            Connection.command.Parameters.AddWithValue("@code", codeTxt.Text);
                        ...
                        {
                            DialogResult avaiable = MessageBox.Show("Kode Satuan " + codeTxt.Text + " sudah terpakai", "Duplicate", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            if (avaiable == DialogResult.Yes)
                            {
                                this.Close();
                            }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                Connection.ConnectionClose();
                Connection.ConnectionOpen();
                string queryCheck = "DELETE FROM UNIT WHERE id=@id";
                Connection.command = new OleDbCommand(queryCheck, Connection.conn);
                Connection.command.CommandType = CommandType.Text;
                Connection.command.Parameters.AddWithValue("@id", idgudang.Text);
        ...
        private void findBtn_Click(object sender, EventArgs e)
        {
            ...
            fieldTable.Add("name", "Nama Satuan");
            fieldTable.Add("code", "Kode Satuan");

            int promptValue = SearchUnit.ShowDialog("Cari Satuan", fieldTable);
