...
namespace inventory.searching
{
    public static class SearchCustomer
    {
        public static int ShowDialog(string Judul, Hashtable fieldTable)
        {
            ...
            string tablename = "CUSTOMER CS LEFT JOIN GROUP_CUSTOMER GC ON GC.ID = CS.GROUP_CUSTOMER_ID";
            string fieldname = "CS.ID, CS.CODE, CS.NAME, CS.ADDRESS, CS.PLAFON, CS.PLAFON_FLAG, GC.NAME AS GROUPCUSTOMER,GC.ID AS GROUPCUSTOMERID";
            ...
            while (list.Read())
            {
                dataGridView2.Rows.Add(list["ID"], list["Code"], list["Name"], list["groupcustomer"], list["plafon"], list["plafon_flag"], list["address"]);
            }
            ...
            searchTextTxt.TextChanged += (s, e) =>
            {
                ...
                {
                    dataGridView2.Rows.Add(autocomplete["ID"], autocomplete["Code"], autocomplete["Name"], autocomplete["groupcustomer"], autocomplete["plafon"], autocomplete["plafon_flag"], autocomplete["address"]);
                }
            };
