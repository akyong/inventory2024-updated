...
{
    public static class SearchDeliveryOrder
    {

        public static int ShowDialog(string Judul, Hashtable fieldTable)
        {
            int liSize = fieldTable.Count;
            string tablename = " ((DELIVERYORDER SO LEFT JOIN WAREHOUSE WF ON WF.ID = SO.WAREHOUSE_ID) LEFT JOIN CUSTOMER CS ON CS.ID = SO.CUSTOMER_ID)LEFT OUTER JOIN PAJAK PJ ON PJ.ID = SO.PAJAK_ID ";
            string fieldname = " SO.ID, SO.DELIVERY_NO, SO.DELIVERY_DATE, WF.NAME AS WAREHOUSE, CS.NAME AS CUSTOMER, SO.DESCRIPTION, PJ.NAME AS PAJAK, SO.TOTALAMOUNT, SO.SO_NUMBER ";
            ...
            coloumOrderNo.DataPropertyName = "so_number";
            coloumOrderNo.HeaderText = "Sales Order";
            ...
            sqlQuery = "SELECT " + fieldname + " FROM " + tablename + " where SO.delete_flag='N' ORDER BY SO.DELIVERY_DATE  DESC";
            ...
            while (list.Read())
            {
                dataGridView2.Rows.Add(list["ID"], list["DELIVERY_NO"], list["DELIVERY_DATE"], list["CUSTOMER"], list["WAREHOUSE"], list["TOTALAMOUNT"], list["DESCRIPTION"], list["SO_NUMBER"]);
            }
            ...
            searchTextTxt.TextChanged += (s, e) =>
            {
                ...
                {
                    dataGridView2.Rows.Add(autocomplete["ID"], autocomplete["DELIVERY_NO"], autocomplete["DELIVERY_DATE"], autocomplete["CUSTOMER"], autocomplete["WAREHOUSE"], autocomplete["TOTALAMOUNT"], autocomplete["DESCRIPTION"], autocomplete["SO_NUMBER"]);
                }
            };
