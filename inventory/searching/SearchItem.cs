...
namespace inventory.searching
{
    public static class SearchItem
    {
        public static int ShowDialog(string Judul, Hashtable fieldTable)
        {
            ...
            string tablename = " ((ITEM  I LEFT JOIN UNIT U ON I.UNIT_ID = U.ID ) LEFT JOIN CATEGORY C ON I.CATEGORY_ID = C.ID) LEFT JOIN GROUP_ITEM G ON I.GROUP_ITEM_ID = G.ID ";
            string fieldname = "I.ID, I.CODE,I.NAME, I.PRICE,I.DELETE_FLAG, U.CODE AS UNIT, C.CODE AS CATEGORY, G.CODE AS GROUPITEM,U.ID AS UID, C.ID AS CID, G.ID AS GID,I.DESCRIPTION ";
            ...
            DataGridViewTextBoxColumn columnHarga = new DataGridViewTextBoxColumn();
            columnHarga.DefaultCellStyle.Format = "N";
            columnHarga.DataPropertyName = "price";
            columnHarga.HeaderText = "Harga";
            columnHarga.Width =70;

            DataGridViewTextBoxColumn columnUnit = new DataGridViewTextBoxColumn();
            columnUnit.DataPropertyName = "unit";
            columnUnit.HeaderText = "Satuan";
            columnUnit.Width = 40;

            DataGridViewTextBoxColumn columnGroupItem = new DataGridViewTextBoxColumn();
            columnGroupItem.DataPropertyName = "groupitem";
            columnGroupItem.HeaderText = "Kelompok";
            columnUnit.Width = 50;
            ...
            dataGridView2.Columns.Add(columnHarga);
            dataGridView2.Columns.Add(columnUnit);
            dataGridView2.Columns.Add(columnGroupItem);
            dataGridView2.Columns.Add(columnCategory);
            dataGridView2.Columns.Add(coloumDescription);

            dataGridView2.Rows.Clear();

            sqlQuery = "SELECT " + fieldname + " FROM " + tablename + " where i.delete_flag='N'";

            OleDbDataReader list = ResultTable.updateTable(sqlQuery, dataGridView2);

            while (list.Read())
            {
                dataGridView2.Rows.Add(list["ID"], list["Code"], list["Name"], list["Price"],list["Unit"], list["GroupItem"], list["Category"], list["Description"]);
            }
            ...
            searchTextTxt.TextChanged += (s, e) =>
            {
                ...
                {
                    dataGridView2.Rows.Add(autocomplete["ID"], autocomplete["Code"], autocomplete["Name"], autocomplete["Price"],autocomplete["Unit"], autocomplete["GroupItem"], autocomplete["Category"], autocomplete["Description"]);
                }
            };
