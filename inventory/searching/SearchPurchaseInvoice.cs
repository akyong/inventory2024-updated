...
{
    public static class SearchPurchaseInvoice
    {
        public static int ShowDialog(string Judul, Hashtable fieldTable)
        {
            int liSize = fieldTable.Count;
            string tablename = " ((PURCHASEINVOICE SO LEFT JOIN WAREHOUSE WF ON WF.ID = SO.WAREHOUSE_ID) LEFT JOIN SUPPLIER CS ON CS.ID = SO.SUPPLIER_ID)LEFT OUTER JOIN PAJAK PJ ON PJ.ID = SO.PAJAK_ID ";
            string fieldname = " SO.ID, SO.INVOICE_NO, SO.INVOICE_DATE, WF.NAME AS WAREHOUSE, CS.NAME AS SUPPLIER, SO.DESCRIPTION, PJ.NAME AS PAJAK, SO.TOTALAMOUNT, SO.RI_NUMBER,SO.PO_NO ";
            ...
            dataGridView2.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.Fill);
            ...
            coloumnobuk.DataPropertyName = "invoice_no";
            coloumnobuk.HeaderText = "No. Invoice";
            coloumnobuk.Width = 80;

            DataGridViewTextBoxColumn coloumDate = new DataGridViewTextBoxColumn();
            coloumDate.DataPropertyName = "invoice_date";
            ...
            coloumOrderNo.DataPropertyName = "ri_number";
            coloumOrderNo.HeaderText = "Receive Number";
            coloumOrderNo.Width = 80;

            DataGridViewTextBoxColumn poNo = new DataGridViewTextBoxColumn();
            poNo.DataPropertyName = "po_no";
            poNo.HeaderText = "No. PO";
            poNo.Width = 80;
            

            //coloumDeleteFlag.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;

            dataGridView2.ColumnDividerDoubleClick += (sender, e) =>
            {
                if (e.Button == MouseButtons.Left)
                {
                    dataGridView2.AutoResizeColumn(e.ColumnIndex, DataGridViewAutoSizeColumnMode.DisplayedCells);
                }
                e.Handled = true;
            };
            ...
            dataGridView2.Columns.Add(poNo);


            dataGridView2.Rows.Clear();

            sqlQuery = "SELECT " + fieldname + " FROM " + tablename + " where SO.delete_flag='N' ORDER BY SO.INVOICE_DATE  DESC";
            ...
            while (list.Read())
            {
                dataGridView2.Rows.Add(list["ID"], list["INVOICE_NO"], list["INVOICE_DATE"], list["SUPPLIER"], list["WAREHOUSE"], list["TOTALAMOUNT"], list["DESCRIPTION"], list["RI_NUMBER"]);
            }
            ...
            searchTextTxt.TextChanged += (s, e) =>
            {
                ...
                {
                    dataGridView2.Rows.Add(autocomplete["ID"], autocomplete["INVOICE_NO"], autocomplete["INVOICE_DATE"], autocomplete["SUPPLIER"], autocomplete["WAREHOUSE"], autocomplete["TOTALAMOUNT"], autocomplete["DESCRIPTION"], autocomplete["RI_NUMBER"]);
                }
            };
