...
namespace inventory.searching
{
    public static class SearchPurchaseOrder
    {
        ...
        public static int ShowDialog(string Judul, Hashtable fieldTable)
        {
            ...
            string tablename = " ((PURCHASEORDER SO LEFT JOIN WAREHOUSE WF ON WF.ID = SO.WAREHOUSE_ID) LEFT JOIN SUPPLIER CS ON CS.ID = SO.SUPPLIER_ID)LEFT OUTER JOIN PAJAK PJ ON PJ.ID = SO.PAJAK_ID ";
            string fieldname = " SO.ID, SO.ORDER_NO, SO.ORDER_DATE, WF.NAME AS WAREHOUSE, CS.NAME AS SUPPLIER, SO.DESCRIPTION, PJ.NAME AS PAJAK, SO.TOTALAMOUNT ";
            ...
            while (list.Read())
            {
                dataGridView2.Rows.Add(list["ID"], list["ORDER_NO"], list["ORDER_DATE"], list["SUPPLIER"], list["WAREHOUSE"], list["TOTALAMOUNT"],list["DESCRIPTION"]);
            }
            ...
            searchTextTxt.TextChanged += (s, e) =>
            {
                ...
                {
                    dataGridView2.Rows.Add(autocomplete["ID"], autocomplete["ORDER_NO"], autocomplete["ORDER_DATE"], autocomplete["SUPPLIER"], autocomplete["WAREHOUSE"], autocomplete["TOTALAMOUNT"], autocomplete["DESCRIPTION"]);
                }
            };
