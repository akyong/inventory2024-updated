...
{
    public static class SearchSalesInvoice
    {
        public static int ShowDialog(string Judul, Hashtable fieldTable)
        {
            int liSize = fieldTable.Count;
            string tablename = " ((INVOICE SO LEFT JOIN WAREHOUSE WF ON WF.ID = SO.WAREHOUSE_ID) LEFT JOIN CUSTOMER CS ON CS.ID = SO.CUSTOMER_ID)LEFT OUTER JOIN PAJAK PJ ON PJ.ID = SO.PAJAK_ID ";
            string fieldname = " SO.ID, SO.INVOICE_NO, SO.INVOICE_DATE, WF.NAME AS WAREHOUSE, CS.NAME AS CUSTOMER, SO.DESCRIPTION, PJ.NAME AS PAJAK, SO.TOTALAMOUNT, SO.DO_NUMBER ";
            ...
            columnFrom.DataPropertyName = "customer";
            columnFrom.HeaderText = "Customer";
            columnFrom.Width = 80;
            ...
            coloumOrderNo.DataPropertyName = "do_number";
            coloumOrderNo.HeaderText = "Delivery Order";
            ...
            while (list.Read())
            {
                dataGridView2.Rows.Add(list["ID"], list["INVOICE_NO"], list["INVOICE_DATE"], list["CUSTOMER"], list["WAREHOUSE"], list["TOTALAMOUNT"], list["DESCRIPTION"], list["DO_NUMBER"]);
            }
            ...
            searchTextTxt.TextChanged += (s, e) =>
            {
                ...
                {
                    dataGridView2.Rows.Add(autocomplete["ID"], autocomplete["INVOICE_NO"], autocomplete["INVOICE_DATE"], autocomplete["CUSTOMER"], autocomplete["WAREHOUSE"], autocomplete["TOTALAMOUNT"], autocomplete["DESCRIPTION"], autocomplete["DO_NUMBER"]);
                }
            };
