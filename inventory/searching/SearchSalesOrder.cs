...
{
    public static class SearchSalesOrder
    {

        public static int ShowDialog(string Judul, Hashtable fieldTable)
        {
            int liSize = fieldTable.Count;
            string tablename = " ((SALESORDER SO LEFT JOIN WAREHOUSE WF ON WF.ID = SO.WAREHOUSE_ID) LEFT JOIN CUSTOMER CS ON CS.ID = SO.CUSTOMER_ID)LEFT OUTER JOIN PAJAK PJ ON PJ.ID = SO.PAJAK_ID ";
            string fieldname = " SO.ID, SO.ORDER_NO, SO.ORDER_DATE, WF.NAME AS WAREHOUSE, CS.NAME AS CUSTOMER, SO.DESCRIPTION, PJ.NAME AS PAJAK, SO.TOTALAMOUNT ";
            ...
            sqlQuery = "SELECT " + fieldname + " FROM " + tablename + " where SO.delete_flag='N' ORDER BY SO.ORDER_DATE  DESC";
            ...
            while (list.Read())
            {
                dataGridView2.Rows.Add(list["ID"], list["ORDER_NO"], list["ORDER_DATE"], list["CUSTOMER"], list["WAREHOUSE"], list["TOTALAMOUNT"],list["DESCRIPTION"]);
            }
            ...
            searchTextTxt.TextChanged += (s, e) =>
            {
                ...
                {
                    dataGridView2.Rows.Add(autocomplete["ID"], autocomplete["ORDER_NO"], autocomplete["ORDER_DATE"], autocomplete["CUSTOMER"], autocomplete["WAREHOUSE"], autocomplete["TOTALAMOUNT"], autocomplete["DESCRIPTION"]);
                }
            };
